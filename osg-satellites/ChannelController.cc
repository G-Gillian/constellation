    ...
    case 2:
        ...
        for (int i = 0; i < (int)satellites.size(); ++i) {
            for (int j = 0; j < (int)stations.size(); ++j)
                addLineOfSight(satellites[i]->getLocatorNode(), satellites[i]->getFullName(), stations[j]->getLocatorNode(), stations[j]->getFullName(),0);
            for (int j = i+1; j < (int)satellites.size(); ++j)
                addLineOfSight(satellites[i]->getLocatorNode(), satellites[i]->getFullName(), satellites[j]->getLocatorNode(), satellites[j]->getFullName(), 1);
        }
        break;
    }
}
...
void ChannelController::addLineOfSight(osg::Node *a, std::string aName, osg::Node *b, std::string bName, int type)
{
    ...
    std::string connection_relation = aName + ":" + bName;
    los->setName(connection_relation);
...
std::vector<std::string> ChannelController::getConnection()
{
...
//                EV << losNode->getName() << "\n";
                connection.push_back(losNode->getName());
            case 1: // sat to sat
                ;
            }
        }
